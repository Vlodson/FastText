...
def count_unique_ngrams(ngram_list: List[str]) -> Dict[str, int]:
    ...
    """
    return {ngram: ngram_list.count(ngram) for ngram in ngram_list}


def count_unique_ngrams_for_unknown_lang(
    unknown_ngram_list: List[str], known_ngram_list: List[str]
) -> Dict[str, int]:
    """
    ...
    """
    return {ngram: unknown_ngram_list.count(ngram) for ngram in known_ngram_list}
...
def standardize_distributions(ngram_counts: Dict[str, int]) -> Dict[str, float]:
    ...
    return {ngram: ngram_counts[ngram] / total_count for ngram in ngram_counts}
...
def kl_divergence(dist1: Dict[str, float], dist2: Dict[str, float]) -> float:
    ...
    return sum(
        [
            dist1[key1] * np.log(dist1[key1]) / np.log(dist2[key2])
            if dist1[key1] != 0
            else 1e2
...
def make_language_ngram_distribution(
    language_corpus: str, ngram_size: int, distribution_path: str
) -> None:
    """
...
def calculate_language_confidence(
    unknown_corpus: str, ngram_size: int, language_distribution_path: str
) -> float:
    """
    Given a corpus of unknown language, calculates the kl divergence w.r.t a known language's distribution
    """
    known_language_dist: Dict[str, float] = deserialize_object(
    ...
    word_list = make_word_list(cleaned)
    ngram_list = word_list_to_ngram_list(word_list, ngram_size)
    ngram_count = count_unique_ngrams_for_unknown_lang(
        ngram_list, list(known_language_dist.keys())
    )
