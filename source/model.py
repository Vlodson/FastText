...
def update_hyperparameter(
    ...
    new_v = OPTIMIZER["omega2"] * v + (1 - OPTIMIZER["omega2"]) * (
    ...
    new_hyperparameter = (
        hyperparameter - (LEARN_RATE / (v_hat + OPTIMIZER["norm"]) ** 0.5) * m_hat
    )
...
def train(
    word_map: Dict[str, Dict[str, np.ndarray]], plot_graph: bool = False
) -> np.ndarray:
    """
